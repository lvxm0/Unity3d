...
public enum GameState { playing,start,over}
...
public class FirstSceneController : MonoBehaviour, ISceneController, IUserAction
{
    ...
    public int round = 1;                                                   //回合
    public float speed = 2f;                                                //发射一个飞碟的时间间隔
    ...
    void Update()
    {
        if (game_start)
        {
            ...
            SendDisk();
            //回合升级
            if(score_recorder.score %10 == 0)
            {
                round = 1 + score_recorder.score / 10;
            }
    ...
    private void SendDisk()
    {
        ...
        if (disk_queue.Count != 0)
        {
            ...
            disk.GetComponent<DiskData>().direction = new Vector3(ran_x, 5f, 0);
            //position_x = Random.Range(14f, 18f);
            position_x = 10f;
            Vector3 position = new Vector3(-disk.GetComponent<DiskData>().direction.x * position_x, 5f, 0);
           ...
           // float angle = Random.Range(15f, 30f);
            ...
            //修改 
            action_manager.playDisk(mode,disk);
        }
